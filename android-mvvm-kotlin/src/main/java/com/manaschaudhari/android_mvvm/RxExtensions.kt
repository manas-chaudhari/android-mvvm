 ...
 * Created by Gabor Szanto on 2017. 03. 24..
 */

/**
 * From ObservableField to Observable
 */
fun <T> ObservableField<T>.asObservable(): Observable<T> = FieldUtils.toObservable(this)

fun <T> ObservableInt.asObservable(): Observable<Int> = FieldUtils.toObservable(this)
fun <T> ObservableFloat.asObservable(): Observable<Float> = FieldUtils.toObservable(this)
fun <T> ObservableDouble.asObservable(): Observable<Double> = FieldUtils.toObservable(this)
fun <T> ObservableLong.asObservable(): Observable<Long> = FieldUtils.toObservable(this)
fun <T> ObservableList<T>.asObservable(): Observable<List<T>> = FieldUtils.toObservable(this)

/**
 * From Observable to ObservableField
 */
fun <T> Observable<T>.asField(): ObservableField<T> = FieldUtils.toField(this)
...
fun <T> Observable<T>.flatMapFirst(obs: Completable): Observable<T> = this
        .toFlowable(io.reactivex.BackpressureStrategy.DROP)
        .flatMapCompletable({ obs.toFlowable(BackpressureStrategy.DROP) }, 1)
        .toObservable()
...
fun Observable<Boolean>.endLoading(end1: Observable<*>, end2: Observable<*>? = null): Observable<Boolean> {
    val end = Observable.merge(end1.map { false }, (end2 ?: Observable.never<Any>()).map { false })
/*    val _end2 = end2?.map { Any() } ?: Observable.just(Any())
    ...
    return Observable.merge(this, end)
}